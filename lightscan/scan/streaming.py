...
class StreamingTCPScanner:
    ...
    async def _scan_with_retries(self, job: _Job) -> ScanResult | None:
        ...
        for attempt in range(self.controls.retries + 1):
            ...
            if status == "closed":
                ...
                return None
            if status == "filtered" and attempt < self.controls.retries:
                continue
            if status == "filtered":
    ...
    async def _connect_once(self, host: str, port: int) -> tuple[str, ScanResult | None]:
        ...
        try:
            ...
            status = "closed" if exc.errno in {errno.ECONNREFUSED} else (
                "filtered" if exc.errno in {errno.ETIMEDOUT, errno.EHOSTUNREACH, errno.ENETUNREACH}
                else "error"
            )
