...
def diff_scans(old_path, new_path):
    def key(r):
        return f"{r.get('target')}:{r.get('port')}:{r.get('module')}:{r.get('status')}"
    ...
    for k, r in appeared.items():
        try:
            sev = Severity[r.get("severity", "INFO")]
        except KeyError:
            sev = Severity.INFO
        results.append(ScanResult(
            f"diff:NEW:{r.get('module', '')}",
            r.get("target", ""),
            r.get("port", 0),
            f"new:{r.get('status', '')}",
            sev,
    ...
    for k, r in resolved.items():
        results.append(ScanResult(
            f"diff:RESOLVED:{r.get('module', '')}",
            r.get("target", ""),
            r.get("port", 0),
            f"resolved:{r.get('status', '')}",
            Severity.INFO,
            f"[RESOLVED] {r.get('detail', '')}"
        ))
