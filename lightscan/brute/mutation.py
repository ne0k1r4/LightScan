...
SUFFIXES = [
    "!", "@", "#", "1", "01", "123", "1234", "12345", "!1",
    "2021", "2022", "2023", "2024", "2025", "!!", "?", "$", "99", "_", ".*"
]
...
class MutationEngine:
    ...
    def _variants(self, word):
        ...
        for s in SUFFIXES:
            yield word + s
            yield word.capitalize() + s
            yield word.upper() + s
        for yr in range(2019, 2026):
            yield f"{word}{yr}"
            yield f"{word.capitalize()}{yr}!"
            yield f"{word}{yr}!"
        yield f"{word.capitalize()}@123"
        yield f"{word.capitalize()}#1"
        yield f"!{word}"
        yield f"{word}!!"
    ...
    def _add(self, cand, seen, result):
    ...
    def generate(self, username="", extra_words=None):
        ...
        if self.include_common:
            for p in COMMON_PASSWORDS:
                self._add(p, seen, result)

        if username:
            for s in SUFFIXES[:10]:
                for cand in (username + s, username.capitalize() + s):
                    self._add(cand, seen, result)
            for yr in range(2020, 2026):
                self._add(f"{username}{yr}", seen, result)

        for base in words:
            count = 0
            for v in self._variants(base):
                if count >= self.max_per_base:
                    break
                if self._add(v, seen, result):
                    count += 1
