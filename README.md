...
lightscan --scan -t 192.0.2.0/28 -p top100 --sv \
  ...
  --retries 1 --host-timeout 10 --format json --output reports
...
| **Streaming TCP discovery** | Uses a bounded queue rather than creating one coroutine per host-port pair. Global and per-host concurrency, optional rate, retry, and host-time limits are explicit. |
...
| **Incremental results and telemetry** | Streams each open finding to NDJSON without retaining all open ports in memory, and writes portable metric snapshots for repeatable throughput and outcome comparisons. |
...
| `--retries N` | Retry count for timeout or filtered outcomes; default: `1`. |
